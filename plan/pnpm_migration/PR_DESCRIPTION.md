...
## 🎯 Executive Summary
This PR refactors the Node.js toolchain installer inside the Librarian Go engine to migrate from the legacy `npm` CLI wrapper to a **Pure, 100% npm-Free pnpm Toolchain** utilizing `corepack` to ensure absolute package linkage determinism.
...
2.  **Complete npm Decoupling**: To eliminate the dependency and runtime footprint of the legacy `npm` binary during tool bootstrapping, we successfully transitioned to Node.js's built-in **`corepack`** wrapper. 
    *   The installer now automatically creates and maps standard executable bin shims globally using **`corepack enable pnpm`** first.
    *   The installer activates the target version globally via **`corepack prepare pnpm@7.32.2 --activate`**.
...
3.  **Clean Global Links**: Replaced `npm link` with **`pnpm link --global`** to bind the compiled typescript generator to the shared global virtual store.
...
*   **[install.go](file:///Users/santiquiroga/Documents/core_client_libraries/node_workspace/reference/librarian/internal/librarian/nodejs/install.go)**: Refactored to bootstrap `pnpm` via `corepack enable pnpm` and `corepack prepare`, dynamically set `global-bin-dir` via Node native `execPath`, and install globally using direct `pnpm add -g` commands (greatly reducing call-site complexity).
...
*   **[librarian.yaml](file:///Users/santiquiroga/Documents/core_client_libraries/node_workspace/reference/librarian/internal/librarian/nodejs/librarian.yaml)**: Migrated tool block key to `pnpm:`, replaced build steps with direct, plain **`pnpm install`** / **`pnpm link --global`** commands, and appended locked global dependencies:
    ```yaml
    - name: protobufjs-cli
      version: "1.2.0"
    - name: protobufjs
...
*   **[install_test.go](file:///Users/santiquiroga/Documents/core_client_libraries/node_workspace/reference/librarian/internal/librarian/nodejs/install_test.go)**: Restored installer tests to stub both the `corepack` and standard `pnpm` wrappers inside `PATH` to test shell block generation pipelines cleanly.
...
```
```
/opt/homebrew/bin/corepack enable pnpm
...
/opt/homebrew/bin/pnpm config set global-bin-dir /Users/santiquiroga/.local/share/mise/installs/node/22.16.0/bin
...
/opt/homebrew/bin/pnpm add -g protobufjs-cli@1.2.0
+ protobufjs-cli 1.2.0
/opt/homebrew/bin/pnpm add -g protobufjs@7.5.5
+ protobufjs 7.5.5
...
ok      github.com/googleapis/librarian/internal/librarian/nodejs 7.207s
