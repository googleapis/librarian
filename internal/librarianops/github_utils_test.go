...
func TestCloneRepoInDir(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if err := cloneRepoInDir(context.Background(), test.repoName, test.repoDir); err != nil {
...
func TestCreatePR(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if err := createPR(context.Background(), test.details); err != nil {
				t.Errorf("unexpected error: %v", err)
			}

			want := [][]string{{"gh", "pr", "create", "--title", test.details.PrTitle, "--body", test.details.PrBody}}
			if diff := cmp.Diff(want, *gotCommands); diff != "" {
				t.Errorf("command mismatch (-want +got):\n%s", diff)
			}
		})
...
func TestCreatePR_Error(t *testing.T) {
	for _, test := range []struct {
		...
		{name: "gh cli error", wantErr: fmt.Errorf("could not create pr")},
	} {
		t.Run(test.name, func(t *testing.T) {
			testRun, _ := testRunCommand(nil, test.wantErr)
			origRunCommand := runCommand
...
func TestGenerateBranchName(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			got := generateBranchName(test.prefix, test.time)
			if got != test.want {
