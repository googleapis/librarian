...
func formatMethodDoxygenComments(m *api.Method, paramComments string, model *api.API, sourceFile string, isPaginated bool, rangeOutputField *api.Field, isLongrunning bool, isRespEmpty bool) string {
	...
	isIamMixin := strings.Contains(m.SourceServiceID, "google.iam.v1.IAMPolicy") || strings.Contains(m.ID, "IAMPolicy")
	if isIamMixin && (strings.HasPrefix(m.InputTypeID, ".google.iam.v1.") || strings.HasPrefix(m.InputTypeID, "google.iam.v1.")) {
		switch m.Name {
		case "SetIamPolicy":
	...
	} else if strings.HasPrefix(doc, "Provides the [") && strings.HasSuffix(doc, "service functionality in this service.") {
		switch m.Name {
		case "GetLocation":
	...
	if m != nil && m.Service != nil {
		depFiles, hasSvcLoc = serviceDependencyFiles(m.Service, model)
	}
	if !hasSvcLoc && sourceFile != "" {
		depFiles = map[string]bool{sourceFile: true}
		hasSvcLoc = true
	}
