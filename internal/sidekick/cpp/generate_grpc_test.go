...
func TestGenerate_SourcesCcPrologue(t *testing.T) {
	outdir := t.TempDir()
	...
	content, err := os.ReadFile(filepath.Join(outdir, "v1", "internal", "echo_sources.cc"))
	if err != nil {
		t.Fatal(err)
	}
	wantPrefix := `// Copyright 2024 Google LLC
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
...
// Generated by the Codegen C++ plugin.
// If you make any local changes, they will be lost.
// source: google/example/echo.proto
`
	if !strings.HasPrefix(string(content), wantPrefix) {
		t.Errorf("missing expected prologue prefix:\nwant prefix:\n%s\ngot:\n%s", wantPrefix, string(content))
	}
}
