...
const (
	...
	// The suffix for the generated long-running operation `Any` converter.
	lroAnyConverterSuffix = "LROAnyConverter"
...
// lroAnyTypes returns the payload types that this service's long-running
...
// The service declares both types up front, so the set is closed and the
	...
	for _, method := range methods {
		...
		for _, id := range []string{
			...
			if !ok {
				...
				// conversion to call. Leaving it out of the converter makes it
				// an unexpected type URL at runtime, which is the same outcome
				// as generating a call that cannot compile.
				continue
			}
...
func (c *codec) annotateLROAnyFields() error {
	if c.LROAnyConverter == "" {
		return nil
	}
	operation, err := lookupMessage(c.Model, longRunningOperationID)
	if err != nil {
