...
cp ./internal/sidekick/internal/config/gcloudyaml/testdata/gcloud.yaml ./test_env/

# 4. Create the .sidekick.toml configuration file.
echo "Creating .sidekick.toml..."
cat > ./test_env/.sidekick.toml << EOL
[general]
language = "gcloud"
specification-format = "protobuf"
specification-source = "."
gcloud-config = "gcloud.yaml"
...
go build -o ./test_env/bin/sidekick-dev ./cmd/sidekick/main.go

echo "Running the gcloud command generator..."
# Run the newly built binary.
(cd  test_env && ./bin/sidekick-dev refresh --output .)
