...
var (
	requiredConfig = map[string]string{"package:google_cloud_gax": "^1.2.3", "package:http": "^4.5.6"}
)
...
func TestCalculateDependencies(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			got, err := calculateDependencies(deps, map[string]string{"http": "^1.3.0"}, "1.2.3")
			if err != nil {
				t.Fatal(err)
			}
