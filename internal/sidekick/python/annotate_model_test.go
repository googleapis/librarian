	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if diff := cmp.Diff(test.want, ann,
				...
				cmpopts.IgnoreFields(serviceAnnotations{}, "Model", "Service", "Methods", "ProtoName", "ClientName", "AsyncClientName", "DocLines", "DirectoryName", "CopyrightYear", "Transport", "Client"),
			); diff != "" {
