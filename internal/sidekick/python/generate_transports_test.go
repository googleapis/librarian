...
func TestGenerateTransports(t *testing.T) {
	outdir := t.TempDir()

	req := api.NewTestMessage("GetSecretRequest")
	resp := api.NewTestMessage("Secret")
	meth := api.NewTestMethod("GetSecret").WithInput(req).WithOutput(resp)
	...
	req.WithSourceLocation("secretmanager.proto", 1)
	resp.WithSourceLocation("secretmanager.proto", 10)
	svc.WithSourceLocation("secretmanager.proto", 20)
	...
	transportsDir := filepath.Join(outdir, "google", "cloud", "secretmanager_v1", "services", "secret_manager_service", "transports")

	t.Run("__init__.py", func(t *testing.T) {
		initPath := filepath.Join(transportsDir, "__init__.py")
...
`
		if diff := cmp.Diff(wantTransInit, initContent); diff != "" {
			t.Errorf("mismatch (-want +got):\n%s", diff)
		}
	})

	t.Run("base.py", func(t *testing.T) {
		basePath := filepath.Join(transportsDir, "base.py")
		baseBytes, err := os.ReadFile(basePath)
		if err != nil {
			t.Fatal(err)
		}
		baseContent := string(baseBytes)
		wantBasePy := `# -*- coding: utf-8 -*-
# Copyright 2026 Google LLC
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
...
from google.cloud.secretmanager_v1.types import secretmanager
...
class SecretManagerServiceTransport(abc.ABC):
    ...
    def get_secret(self) -> Callable[
            ...
            Union[
                ...
                Awaitable[secretmanager.Secret]
            ]]:
        raise NotImplementedError()
...
`
		if diff := cmp.Diff(wantBasePy, baseContent); diff != "" {
			t.Errorf("mismatch (-want +got):\n%s", diff)
		}
	})

	t.Run("grpc.py", func(t *testing.T) {
		grpcPath := filepath.Join(transportsDir, "grpc.py")
		grpcBytes, err := os.ReadFile(grpcPath)
		if err != nil {
			t.Fatal(err)
		}
		grpcContent := string(grpcBytes)
		wantGrpcPy := `# -*- coding: utf-8 -*-
# Copyright 2026 Google LLC
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
    ...
    def get_secret(self) -> Callable[
        ...
        if 'get_secret' not in self._stubs:
            self._stubs['get_secret'] = self._logged_channel.unary_unary(
                ...
                request_serializer=secretmanager.GetSecretRequest.serialize,
                response_deserializer=secretmanager.Secret.deserialize,
            )
...
`
		if diff := cmp.Diff(wantGrpcPy, grpcContent); diff != "" {
			t.Errorf("mismatch (-want +got):\n%s", diff)
		}
	})

	t.Run("grpc_asyncio.py", func(t *testing.T) {
		grpcAsyncPath := filepath.Join(transportsDir, "grpc_asyncio.py")
		grpcAsyncBytes, err := os.ReadFile(grpcAsyncPath)
		if err != nil {
			t.Fatal(err)
		}
		grpcAsyncContent := string(grpcAsyncBytes)
		wantGrpcAsyncPy := `# -*- coding: utf-8 -*-
# Copyright 2026 Google LLC
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
...
`
		if diff := cmp.Diff(wantGrpcAsyncPy, grpcAsyncContent); diff != "" {
			t.Errorf("mismatch (-want +got):\n%s", diff)
		}
	})
}
