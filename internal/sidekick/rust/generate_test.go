...
func TestCodecError(t *testing.T) {
	...
	model, err := parser.CreateModel(toParserConfig(errorConfig), parser.NewModelOverridesFromSource(errorConfig.Source))
	if err != nil {
		t.Fatal(err)
	}
...
func TestRustFromOpenAPI(t *testing.T) {
	...
	model, err := parser.CreateModel(toParserConfig(cfg), parser.NewModelOverridesFromSource(cfg.Source))
	if err != nil {
		t.Fatal(err)
	}
