...
func TestGetGitHubRepoFromRemote(t *testing.T) {
	...
	newTestRepo := func(t *testing.T) *Repository {
		t.Helper()
		dir := t.TempDir()
		gogitRepo, err := git.PlainInit(dir, false)
		if err != nil {
	...
	tests := []struct {
		name       string
		setupRepo  func(t *testing.T, r *Repository)
		...
		{
			name: "single github remote without .git",
			setupRepo: func(t *testing.T, r *Repository) {
				_, err := r.repo.CreateRemote(&config.RemoteConfig{
					Name: "origin",
		...
		},
		{
			...
			setupRepo:  nil,
			wantErr:    true,
			wantErrMsg: "no GitHub remotes found",
		},
		{
			...
			setupRepo: func(t *testing.T, r *Repository) {
				if _, err := r.repo.CreateRemote(&config.RemoteConfig{
					Name: "origin",
					URLs: []string{"https://github.com/owner/repo1.git"},
				}); err != nil {
					t.Fatalf("CreateRemote() failed: %v", err)
				}
				if _, err := r.repo.CreateRemote(&config.RemoteConfig{
			...
			wantErr:    true,
			wantErrMsg: "can only determine the GitHub repo with a single matching remote",
		},
		{
			name: "one github, one non-github remote",
			setupRepo: func(t *testing.T, r *Repository) {
				if _, err := r.repo.CreateRemote(&config.RemoteConfig{
					Name: "origin",
					URLs: []string{"https://github.com/owner/repo.git"},
				...
				if _, err := r.repo.CreateRemote(&config.RemoteConfig{
					Name: "upstream",
					URLs: []string{"https://gitlab.com/owner/repo.git"},
				}); err != nil {
					t.Fatalf("CreateRemote() failed: %v", err)
				}
			},
		...
		{
			name: "remote with no github url",
			setupRepo: func(t *testing.T, r *Repository) {
				if _, err := r.repo.CreateRemote(&config.RemoteConfig{
					Name: "origin",
					URLs: []string{"https://gitlab.com/owner/repo.git"},
		...
		{
			name: "multiple urls, first is not github",
			setupRepo: func(t *testing.T, r *Repository) {
				if _, err := r.repo.CreateRemote(&config.RemoteConfig{
					Name: "origin",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			gotRepo, err := repo.GetGitHubRepoFromRemote(repo)

			if (err != nil) != tt.wantErr {
				t.Errorf("GetGitHubRepoFromRemote() error = %v, wantErr %v", err, tt.wantErr)
				return
			}

			if tt.wantErr {
				if !strings.Contains(err.Error(), tt.wantErrMsg) {
					t.Errorf("GetGitHubRepoFromRemote() error = %q, want to contain %q", err.Error(), tt.wantErrMsg)
				}
				if tt.name == "multiple github remotes" {
					if !strings.Contains(err.Error(), "origin") || !strings.Contains(err.Error(), "upstream") {
						t.Errorf("GetGitHubRepoFromRemote() error = %q, want to contain 'origin' and 'upstream'", err.Error())
					}
				}
				return
			}
			...
			if diff := cmp.Diff(tt.wantRepo, gotRepo); diff != "" {
				t.Errorf("GetGitHubRepoFromRemote() mismatch (-want +got):\n%s", diff)
			}
		})
	}
}
