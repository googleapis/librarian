...
func TestCheckout(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			t.Parallel()
			dir := t.TempDir()
			repo, err := clone(dir, "https://github.com/googleapis/librarian", "main", "true", 0)
			if err != nil {
			...
			commit, err := object.GetCommit(repo.repo.Storer, ref.Hash())
			if err != nil {
				t.Fatalf("Checkout() unexpected error fetching commit: %v", err)
			}
			commitLines := strings.Split(commit.Message, "\n")
			if diff := cmp.Diff(test.want, commitLines[0]); diff != "" {
				t.Fatalf("Checkout() mismatch (-want +got):\n%s", diff)
			}
		})
	}
