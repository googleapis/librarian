...
func NewCommand(method *api.Method, overrides *provider.Config, model *api.API, service *api.Service) (*Command, error) {
	...
	inferredTrack := provider.InferTrackFromPackage(method.Service.Package)
...
func newArguments(method *api.Method, overrides *provider.Config, model *api.API, service *api.Service) (Arguments, error) {
	args := Arguments{}
...
func addFlattenedParams(field *api.Field, prefix string, args *Arguments, overrides *provider.Config, model *api.API, service *api.Service, method *api.Method) error {
	...
	if provider.IsPrimaryResource(field, method) {
...
func newParam(field *api.Field, apiField string, overrides *provider.Config, model *api.API, service *api.Service, method *api.Method) (Param, error) {
	...
	if provider.IsUpdate(method) && param.Repeated {
...
func newPrimaryResourceParam(field *api.Field, method *api.Method, model *api.API, service *api.Service) Param {
	...
	if field.Name == "name" || provider.IsList(method) {
	...
	param := Param{
		HelpText:          helpText,
		IsPositional:      !provider.IsList(method),
...
func newRequest(method *api.Method, overrides *provider.Config, service *api.Service) *Request {
	req := &Request{
...
func newOutputConfig(method *api.Method) *OutputConfig {
	...
	resourceMsg := provider.FindResourceMessage(method.OutputType)
	if resourceMsg == nil {
		return nil
	}
...
func findHelpTextRule(method *api.Method, overrides *provider.Config) *provider.HelpTextRule {
...
func findFieldHelpTextRule(field *api.Field, overrides *provider.Config) *provider.HelpTextRule {
	if overrides.APIs == nil {
		return nil
	}
	for _, api := range overrides.APIs {
...
func apiVersion(overrides *provider.Config) string {
	if len(overrides.APIs) > 0 {
