...
func NewCommand(method *api.Method, apiConfig *API, model *api.API, service *api.Service) (*Command, error) {
	// We look up the help text and API definition for this specific method in the
	// `gcloud.yaml` configuration file.
	rule := findHelpTextRule(method, apiConfig)
	...
	cmd.Hidden = apiConfig.RootIsHidden
	...
	// request details, and async configuration.
	args, err := newArguments(method, apiConfig, model, service)
	if err != nil {
		return nil, err
	}
	cmd.Arguments = args
	cmd.Request = newRequest(method, apiConfig, model, service)
...
func newArguments(method *api.Method, apiConfig *API, model *api.API, service *api.Service) (Arguments, error) {
	args := Arguments{}
	...
	for _, field := range method.InputType.Fields {
		...
		if err := addFlattenedParams(field, field.JSONName, &args, apiConfig, model, service, method); err != nil {
...
func addFlattenedParams(field *api.Field, prefix string, args *Arguments, apiConfig *API, model *api.API, service *api.Service, method *api.Method) error {
	...
	if field.MessageType != nil && !field.Map {
		for _, f := range field.MessageType.Fields {
			if err := addFlattenedParams(f, fmt.Sprintf("%s.%s", prefix, f.JSONName), args, apiConfig, model, service, method); err != nil {
				return err
			}
		}
		return nil
	...
	param, err := newParam(field, prefix, apiConfig, model, service, method)
	if err != nil {
		return err
	}
	args.Params = append(args.Params, param)
...
func newParam(field *api.Field, apiField string, apiConfig *API, model *api.API, service *api.Service, method *api.Method) (Param, error) {
	...
	if rule := findFieldHelpTextRule(field, apiConfig); rule != nil {
...
func newRequest(method *api.Method, apiConfig *API, _ *api.API, service *api.Service) *Request {
	req := &Request{
...
func findHelpTextRule(method *api.Method, apiConfig *API) *HelpTextRule {
	if apiConfig.HelpText == nil {
		return nil
	}
	for _, rule := range apiConfig.HelpText.MethodRules {
...
func findFieldHelpTextRule(field *api.Field, apiConfig *API) *HelpTextRule {
	if apiConfig.HelpText == nil {
		return nil
	}
	for _, rule := range apiConfig.HelpText.FieldRules {
