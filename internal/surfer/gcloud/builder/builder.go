...
// Package builder constructs gcloud command definitions from API methods.
package builder
...
func NewCommand(method *api.Method, overrides *input.Config, model *api.API, service *api.Service) (*output.Command, error) {
	...
	inferredTrack := input.InferTrackFromPackage(method.Service.Package)
...
func newArguments(method *api.Method, overrides *input.Config, model *api.API, service *api.Service) (output.Arguments, error) {
...
func isIgnored(field *api.Field, method *api.Method) bool {
	...
	if input.IsUpdate(method) && slices.Contains(field.Behavior, api.FIELD_BEHAVIOR_IMMUTABLE) {
		return true
	}
...
func addFlattenedParams(field *api.Field, prefix string, args *output.Arguments, overrides *input.Config, model *api.API, service *api.Service, method *api.Method) error {
	...
	if input.IsPrimaryResource(field, method) {
...
func newParam(field *api.Field, apiField string, overrides *input.Config, model *api.API, service *api.Service, method *api.Method) (output.Param, error) {
	...
	if field.ResourceReference != nil {
		...
		for _, v := range field.EnumType.Values {
			...
			param.Choices = append(param.Choices, output.Choice{
				ArgValue:  strcase.ToKebab(v.Name),
				EnumValue: v.Name,
			})
		}
	} else {
		param.Type = input.GetGcloudType(field.Typez)
	}

	if input.IsUpdate(method) && param.Repeated {
...
func newPrimaryResourceParam(field *api.Field, method *api.Method, model *api.API, service *api.Service) output.Param {
	resource := input.GetResourceForMethod(method, model)
	...
	if input.IsList(method) {
		segments = input.GetParentFromSegments(segments)
	}

	resourceName := strings.TrimSuffix(field.Name, "_id")
	if field.Name == "name" || input.IsList(method) {
		resourceName = input.GetSingularFromSegments(segments)
	}

	var helpText string
	switch {
	case input.IsCreate(method):
		helpText = fmt.Sprintf("The %s to create.", resourceName)
	case input.IsList(method):
		helpText = fmt.Sprintf("The project and location for which to retrieve %s information.", input.GetPluralFromSegments(segments))
	default:
		helpText = fmt.Sprintf("The %s to operate on.", resourceName)
	}

	collectionPath := input.GetCollectionPathFromSegments(segments)
	hostParts := strings.Split(service.DefaultHost, ".")
	shortServiceName := hostParts[0]

	param := output.Param{
		HelpText:          helpText,
		IsPositional:      !input.IsList(method),
		IsPrimaryResource: true,
		Required:          true,
		ResourceSpec: &output.ResourceSpec{
			Name:                  resourceName,
			PluralName:            input.GetPluralFromSegments(segments),
...
func newResourceReferenceSpec(field *api.Field, model *api.API, service *api.Service) (*output.ResourceSpec, error) {
	for _, def := range model.ResourceDefinitions {
		if def.Type == field.ResourceReference.Type {
			...
			name := input.GetSingularFromSegments(segments)

			hostParts := strings.Split(service.DefaultHost, ".")
			shortServiceName := hostParts[0]
			baseCollectionPath := input.GetCollectionPathFromSegments(segments)
			fullCollectionPath := fmt.Sprintf("%s.%s", shortServiceName, baseCollectionPath)

			return &output.ResourceSpec{
				Name:       name,
...
func newAttributesFromSegments(segments []api.PathSegment) []output.Attribute {
	var attributes []output.Attribute
...
func newRequest(method *api.Method, overrides *input.Config, service *api.Service) *output.Request {
	req := &output.Request{
	...
	} else if !input.IsStandardMethod(method) {
		commandName, _ := input.GetCommandName(method)
...
func newAsync(method *api.Method, model *api.API, service *api.Service) *output.Async {
	async := &output.Async{
	...
	singular := input.GetSingularResourceNameForMethod(method, model)
...
func newCollectionPath(method *api.Method, service *api.Service, isAsync bool) []string {
	...
	for _, binding := range method.PathInfo.Bindings {
		...
		basePath := input.ExtractPathFromSegments(binding.PathTemplate.Segments)
...
func newOutputConfig(method *api.Method) *output.OutputConfig {
	if !input.IsList(method) {
		return nil
	}

	resourceMsg := input.FindResourceMessage(method.OutputType)
	if resourceMsg == nil {
		return nil
	}
...
func newFormat(message *api.Message) string {
	...
	for _, f := range message.Fields {
		...
		if !input.IsSafeName(f.JSONName) {
			continue
		}
...
func findHelpTextRule(method *api.Method, overrides *input.Config) *input.HelpTextRule {
...
func findFieldHelpTextRule(field *api.Field, overrides *input.Config) *input.HelpTextRule {
	if overrides.APIs == nil {
		return nil
	}
	for _, api := range overrides.APIs {
...
func apiVersion(overrides *input.Config) string {
	if len(overrides.APIs) > 0 {
