...
func writeCommandGroupTree(outputDir string, root *CommandGroup) error {
	if root == nil {
		return nil
	}

	groupDir := filepath.Join(outputDir, strcase.ToSnake(root.Name))
	if err := os.MkdirAll(groupDir, 0755); err != nil {
		return err
	}

	if err := writeCommandGroupFile(groupDir, *root); err != nil {
		return err
	}

	for name, cmd := range root.Commands {
		if err := writeCommandFiles(groupDir, name, cmd, "unnamed"); err != nil {
			return err
		}
	}

	for _, group := range root.Groups {
		if err := writeCommandGroupTree(groupDir, group); err != nil {
			return err
		}
	}
...
@base.Autogenerated
@base.Hidden
class {{$.Name | toCamel}}{{. | toCamel}}(base.Group):
  """{{$.HelpText}}"""
{{end}}`))
