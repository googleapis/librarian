...
func TestInstall_VendorBin(t *testing.T) {
	testhelper.RequireCommand(t, "composer")
	bin := t.TempDir()
	testhelper.WriteExecutable(t, filepath.Join(bin, "composer"), "#!/bin/sh\nexit 0\n")
	t.Setenv("PATH", bin+string(os.PathListSeparator)+os.Getenv("PATH"))
	...
	localDir := t.TempDir()
	// Create the mock vendor/bin directory and binary
	vendorBinDir := filepath.Join(localDir, "vendor", "bin")
	if err := os.MkdirAll(vendorBinDir, 0o755); err != nil {
		t.Fatal(err)
	}
	testhelper.WriteExecutable(t, filepath.Join(vendorBinDir, "my-tool"), "")
	...
	wrapperPath := filepath.Join(binDir, "my-tool")
	b, err := os.ReadFile(wrapperPath)
	if err != nil {
		t.Fatal(err)
	}
	destPath := filepath.Join(vendorBinDir, "my-tool")
	want := phpWrapperContent("php", destPath)
	if diff := cmp.Diff(want, string(b)); diff != "" {
		t.Errorf("mismatch (-want +got):\n%s", diff)
	}
