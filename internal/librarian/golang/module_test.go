...
const (
	mockLibraryGoMod = `module example.com/testtidy
go 1.22.0
`
)
...
func TestTidyModule(t *testing.T) {
	...
	for _, test := range []struct {
		name        string
		setupModule func(t *testing.T, outDir string)
		library     *config.Library
	}{
		{
			name: "tidy only output directory",
			setupModule: func(t *testing.T, outDir string) {
				if err := os.WriteFile(filepath.Join(outDir, "go.mod"), []byte(mockLibraryGoMod), 0644); err != nil {
					t.Fatal(err)
				}
			},
			library: &config.Library{
		...
		t.Run(test.name, func(t *testing.T) {
			outDir := filepath.Join(t.TempDir(), "testtidy")
			if err := os.MkdirAll(outDir, 0755); err != nil {
				t.Fatal(err)
			}
			test.library.Output = outDir
			test.setupModule(t, outDir)
			if err := TidyModule(t.Context(), test.library, command.Go); err != nil {
				t.Fatal(err)
			}
		})
	}
