...
func TestRunPostProcessor_CustomScripts_RootRelativePath(t *testing.T) {
	...
	// Create dummy staging dir so combine-library doesn't fail
	stagingDir := filepath.Join(repoRoot, "owl-bot-staging", library.Name, "v1")
	if err := os.MkdirAll(filepath.Join(stagingDir, "src"), 0755); err != nil {
		t.Fatal(err)
	}
