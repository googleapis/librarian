...
func (r *generateRunner) run(ctx context.Context) error {
	...
	if r.api != "" || r.library != "" {
		...
		if failedGenerations > 0 && failedGenerations+skippedGenerations == len(r.state.Libraries) {
			return fmt.Errorf("all %d libraries failed to generate (skipped: %d)",
				failedGenerations, skippedGenerations)
		}
	}
...
// higher up in run. If this function returns a non-nil error or indicates that
// a library should not be generated, it always logs the cause; no further logging
// is required by the caller.
//
// This function observes the following logic:
//
//   - If the library has a manual configuration which indicates it's blocked, the
//     the library is skipped.
//   - If the library has no APIs, it is skipped.
//   - If r.generatedUnchanged is true (usually via the -generate-unchanged flag)
//     then the library is generated.
//   - If the library does not have a last_generated_commit then the library is generated.
//   - If any API (as indicated by API.Path) has changed between the last_generated_commit
//     and the HEAD commit of r.sourceRepo have changed, then the library is generated.
//   - Otherwise, the library is skipped.
func (r *generateRunner) shouldGenerate(library *config.LibraryState) (bool, error) {
	...
	// We don't need to find which APIs have changed if we've been asked to
	// generate even unchanged ones.
	if r.generateUnchanged {
		return true, nil
	}
	// We can't find out which APIs have changed if we don't know at which
	// commit the library was last generated.
	if library.LastGeneratedCommit == "" {
		return true, nil
	}
	headHash, err := r.sourceRepo.HeadHash()
	if err != nil {
		slog.Error("failed to get head hash for source repo", "library", library.ID)
		return false, err
	}
	for _, api := range library.APIs {
		oldHash, err := r.sourceRepo.GetHashForPathOrEmpty(library.LastGeneratedCommit, api.Path)
		if err != nil {
