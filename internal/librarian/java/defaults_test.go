...
func TestTidyKeep(t *testing.T) {
	...
	for _, test := range []struct {
		...
		keep       []string
		want       []string
		setupFiles func(t *testing.T, baseDir string)
	}{
		{
			name: "nil keep",
			keep: nil,
		...
		{
			// Rule 1: If a file is NOT in a GAPIC module (e.g., in a Proto or GRPC module), it is NOT redundant.
			name: "Rule 1: not in GAPIC module",
			keep: []string{
				"proto-google-cloud-vision-v1/src/main/java/com/google/cloud/vision/v1/ImageName.java",
				"grpc-google-cloud-vision-v1/src/main/java/com/google/cloud/vision/v1/Stub.java",
			},
			want: []string{
				"grpc-google-cloud-vision-v1/src/main/java/com/google/cloud/vision/v1/Stub.java",
				"proto-google-cloud-vision-v1/src/main/java/com/google/cloud/vision/v1/ImageName.java",
		...
		{
			// Rule 2: If it's outside of the GAPIC module's src/ directory, it won't be cleaned, so it IS redundant.
			name: "Rule 2: outside of GAPIC module src directory",
			keep: []string{
				"google-cloud-vision/README.md",
				"google-cloud-vision/samples/snippets/generated/Foo.java",
			},
			want: nil,
		},
		{
			// Rule 3: If it's a file that is in the generateIfMissing list (like pom.xml), it is never cleaned, so it IS redundant.
			name: "Rule 3: file in generateIfMissing list",
			keep: []string{
				"google-cloud-vision/src/main/resources/pom.xml",
				"google-cloud-vision/src/main/resources/clirr-ignored-differences.xml",
			},
			want: nil,
		},
		{
			// Rule 4: If it's a generated but non-Java file (in generatedNonJavaFiles), it is cleaned unconditionally, so it is NOT redundant to keep it.
			name: "Rule 4: generated but non-Java file in GAPIC src",
			keep: []string{
				"google-cloud-vision/src/main/resources/META-INF/native-image/com.google.cloud.vision/reflect-config.json",
				"google-cloud-vision/src/main/resources/gapic_metadata.json",
			},
			want: []string{
				"google-cloud-vision/src/main/resources/META-INF/native-image/com.google.cloud.vision/reflect-config.json",
				"google-cloud-vision/src/main/resources/gapic_metadata.json",
		...
		{
			// Rule 4 (negative): If it's a non-Java file NOT in the generated list, it is not cleaned, so keeping it IS redundant.
			name: "Rule 4 negative: non-Java file not in generated list",
			keep: []string{
				"google-cloud-vision/src/test/resources/placeholder.txt",
			},
			want: nil,
		...
		{
			// Rule 5a: If a Java file does NOT have an autogenerated marker (handwritten/manual), it is not cleaned, so it IS redundant.
			name: "Rule 5a: Java file without autogenerated marker",
			keep: []string{
				"google-cloud-vision/src/main/java/com/google/cloud/vision/v1/Handwritten.java",
			},
			want: nil,
			setupFiles: func(t *testing.T, baseDir string) {
				hwPath := filepath.Join(baseDir, "google-cloud-vision/src/main/java/com/google/cloud/vision/v1/Handwritten.java")
				if err := os.MkdirAll(filepath.Dir(hwPath), 0755); err != nil {
					t.Fatal(err)
				}
				if err := os.WriteFile(hwPath, []byte("package com.google.cloud.vision.v1;\npublic class Handwritten {}"), 0644); err != nil {
					t.Fatal(err)
				}
			},
		},
		{
			// Rule 5b: If a Java file HAS an autogenerated marker, it is cleaned by the clean tool, so it is NOT redundant to keep it.
			name: "Rule 5b: Java file with autogenerated marker",
			keep: []string{
			...
			setupFiles: func(t *testing.T, baseDir string) {
				genPath := filepath.Join(baseDir, "google-cloud-vision/src/main/java/com/google/cloud/vision/v1/Generated.java")
				if err := os.MkdirAll(filepath.Dir(genPath), 0755); err != nil {
					t.Fatal(err)
				}
				if err := os.WriteFile(genPath, []byte("// AUTO-GENERATED DOCUMENTATION AND CLASS.\npackage com.google.cloud.vision.v1;\npublic class Generated {}"), 0644); err != nil {
					t.Fatal(err)
				}
			},
		},
		{
			// Rule 5c: If the Java file does not exist, we cannot check for the marker, so we err on the side of caution and treat it as NOT redundant (keep it).
			name: "Rule 5c: Java file that does not exist",
			keep: []string{
				"google-cloud-vision/src/main/java/com/google/cloud/vision/v1/Missing.java",
			},
			want: []string{
				"google-cloud-vision/src/main/java/com/google/cloud/vision/v1/Missing.java",
			},
		...
		{
			// Default preserved files (like IT*Test.java or Version.java) are automatically preserved by the clean tool, so keeping them is redundant.
			name: "Default preserved files",
			keep: []string{
				"google-cloud-vision/src/test/java/com/google/cloud/vision/it/ITSystemTest.java",
				"google-cloud-vision/src/main/java/com/google/cloud/vision/v1/stub/Version.java",
			},
			want: nil,
			setupFiles: func(t *testing.T, baseDir string) {
				itPath := filepath.Join(baseDir, "google-cloud-vision/src/test/java/com/google/cloud/vision/it/ITSystemTest.java")
				if err := os.MkdirAll(filepath.Dir(itPath), 0755); err != nil {
					t.Fatal(err)
				}
				if err := os.WriteFile(itPath, []byte("package com.google.cloud.vision.it;\npublic class ITSystemTest {}"), 0644); err != nil {
					t.Fatal(err)
				}
			},
		},
