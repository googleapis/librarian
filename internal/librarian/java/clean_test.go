...
func TestClean_Marker(t *testing.T) {
	for _, test := range []struct {
		name     string
		fileName string
		content  string
		want     bool // true if expected to be kept
	}{
		{
		...
		t.Run(test.name, func(t *testing.T) {
			...
			libraryName := "google-cloud-secretmanager"
			gapicSrc := filepath.Join(tmpDir, libraryName, "src")
			if err := os.MkdirAll(gapicSrc, 0755); err != nil {
				t.Fatal(err)
			}

			path := filepath.Join(gapicSrc, test.fileName)
			...
			if got != test.want {
				t.Errorf("file exists = %v, want %v", got, test.want)
			}
		})
	}
}
