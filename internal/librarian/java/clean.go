...
const autoGeneratedMarker = "// AUTO-GENERATED DOCUMENTATION AND CLASS."
...
func cleanPath(targetPath, root string, keepSet map[string]bool, useMarker bool) error {
	...
	err := filepath.WalkDir(targetPath, func(path string, d os.DirEntry, err error) error {
		...
		if useMarker {
			if d.Name() == "gapic_metadata.json" || d.Name() == "reflect-config.json" {
				return os.Remove(path)
			}
			if filepath.Ext(path) != ".java" {
				return nil
			}
			found, err := hasMarker(path)
			if err != nil {
				return err
			}
			if !found {
...
// It scans the file line by line and returns true as soon as the marker is found,
// avoiding reading the entire file for generated classes.
func hasMarker(path string) (found bool, err error) {
	...
	for scanner.Scan() {
		if strings.Contains(scanner.Text(), autoGeneratedMarker) {
			return true, nil
		}
	}
