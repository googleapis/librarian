...
func TestCreateArgFile(t *testing.T) {
	for _, test := range []struct {
		name  string
		files []string
		want  string
		...
		{
			name:  "single file",
			files: []string{"/path/to/ClassA.java"},
			want:  "--replace\n/path/to/ClassA.java",
		},
		{
			name:  "multiple files",
			files: []string{"/path/to/ClassA.java", "/path/to/ClassB.java"},
			want:  "--replace\n/path/to/ClassA.java\n/path/to/ClassB.java",
		},
		{
		...
		t.Run(test.name, func(t *testing.T) {
			argFile, err := createArgFile(test.files)
			if err != nil {
				t.Fatal(err)
			}
