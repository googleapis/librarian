...
latest API definitions is:
	...
	librarian generate --all`,
		...
		Action: func(ctx context.Context, cmd *cli.Command) error {
			...
			if err := runGenerate(ctx, cfg, all, libraryName); err != nil {
				return err
			}
			return writeLockFile(cfg)
		},
	}
}

func writeLockFile(cfg *config.Config) error {
	b, err := yaml.Marshal(cfg)
	if err != nil {
		return err
	}
	hash := sha256.Sum256(b)
	content := fmt.Sprintf(`# .librarian.lock
# This file is generated by librarian generate. Do not edit.

regen_time: %s
config_checksum: %x
`, time.Now().UTC().Format(time.RFC3339), hash)
	return os.WriteFile(".librarian.lock", []byte(content), 0644)
}
