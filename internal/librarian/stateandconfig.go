...
func loadRepoPipelineState(languageRepo *gitrepo.Repository) (*statepb.PipelineState, error) {
	path := filepath.Join(languageRepo.Dir, constants.GeneratorInputDir, pipelineStateFile)
...
func loadRepoPipelineConfig(languageRepo *gitrepo.Repository) (*statepb.PipelineConfig, error) {
	path := filepath.Join(languageRepo.Dir, constants.GeneratorInputDir, "pipeline-config.json")
...
func savePipelineState(state *commandState) error {
	path := filepath.Join(state.languageRepo.Dir, constants.GeneratorInputDir, pipelineStateFile)
...
func fetchRemotePipelineState(ctx context.Context, repo *githubrepo.Repository, ref string, gitHubToken string) (*statepb.PipelineState, error) {
	...
	return parsePipelineState(func() ([]byte, error) {
		return ghClient.GetRawContent(ctx, repo, constants.GeneratorInputDir+"/"+pipelineStateFile, ref)
	})
}
