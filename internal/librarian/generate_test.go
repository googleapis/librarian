...
func TestRestoreLibrary(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			// Create library files in backup directory.
			for _, path := range test.sourceRoots {
				relPath := filepath.Join(r.backupDir, test.libraryID, path)
				if err := os.MkdirAll(relPath, 0755); err != nil {
					t.Error(err)
				}

				file := filepath.Join(relPath, "example.txt")
				if err := os.WriteFile(file, []byte("old contents"), 0755); err != nil {
					t.Error(err)
				}
			}
			// Create library files in repo directory with new files and contents.
			for _, path := range test.sourceRoots {
				...
				file := filepath.Join(relPath, "example.txt")
				if err := os.WriteFile(file, []byte("new contents"), 0755); err != nil {
					t.Error(err)
				}

				file = filepath.Join(relPath, "another-example.txt")
				if err := os.WriteFile(file, []byte("new contents"), 0755); err != nil {
					t.Error(err)
				}
			}

			err := r.restoreLibrary(test.libraryID)
			if test.wantErr {
				if err == nil {
					t.Errorf("%s should return error", test.name)
				}
			...
			// Verify library files exist in backup dir.
			for _, path := range test.sourceRoots {
				expectedFile := filepath.Join(r.repo.GetDir(), path, "example.txt")
				_, err := os.Stat(expectedFile)
				if err != nil {
					t.Fatalf("backup failed for %s", expectedFile)
				}
				file, err := os.ReadFile(expectedFile)
				if err != nil {
					t.Fatal(err)
				}
				if diff := cmp.Diff("old contents", string(file)); diff != "" {
					t.Errorf("%s: file content mismatch (-want +got):%s", test.name, diff)
				}

				expectedFile = filepath.Join(r.repo.GetDir(), path, "another-example.txt")
				if _, err := os.Stat(expectedFile); !os.IsNotExist(err) {
					t.Fatalf("%s should not exist", expectedFile)
				}
			}
		})
	}
