...
func PublishCrates(ctx context.Context, cfg *config.Release, dryRun bool, skipSemverChecks bool, lastTag string, files []string) error {
	...
	if cfg.RootsPem != "" {
		cmd.Env = append(os.Environ(), fmt.Sprintf("CARGO_HTTP_CAINFO=%s", cfg.RootsPem))
	}
	cmd.Dir = "."
