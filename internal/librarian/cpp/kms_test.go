...
func TestKms(t *testing.T) {
	...
	libraries := []*config.Library{
		{
			Name:                "kms_autokey",
			Output:              outDir,
			APIs:                []*config.API{{Path: "google/cloud/kms/v1"}},
			...
			Cpp: &config.CppLibrary{
				...
				OmittedServices:      []string{"AutokeyAdmin", "EkmService", "KeyManagementService"},
			},
		},
		{
			Name:                "kms_autokey_admin",
			Output:              outDir,
			APIs:                []*config.API{{Path: "google/cloud/kms/v1"}},
			SpecificationFormat: config.SpecProtobuf,
			Cpp: &config.CppLibrary{
				...
				InitialCopyrightYear: "2024",
				RetryableStatusCodes: []string{"kUnavailable"},
				OmittedServices:      []string{"Autokey", "EkmService", "KeyManagementService"},
			},
		},
		{
			Name:                "kms_key_management",
			Output:              outDir,
			APIs:                []*config.API{{Path: "google/cloud/kms/v1"}},
			SpecificationFormat: config.SpecProtobuf,
			Cpp: &config.CppLibrary{
				...
				RetryableStatusCodes:  []string{"kUnavailable"},
				OmittedServices:       []string{"Autokey", "AutokeyAdmin", "EkmService"},
			},
		},
		{
			Name:                "kms_ekm",
			...
			Cpp: &config.CppLibrary{
				...
				InitialCopyrightYear:  "2022",
				RetryableStatusCodes:  []string{"kUnavailable"},
				OmittedServices:       []string{"Autokey", "AutokeyAdmin", "KeyManagementService"},
			},
		},
	}
	...
	if err := Format(t.Context(), cfg, &config.Library{Output: outDir}); err != nil {
		t.Fatal(err)
	}
	...
	refBase := filepath.Join(cppRepo, "google", "cloud", "kms")
	...
	// The 122 expected files comprise public headers and sources across 4 services
	// (autokey, autokey_admin, key_management, ekm), their internal stubs,
	// decorators, and sources, plus forwarding headers and mocks:
	// - 8 forwarding headers under google/cloud/kms/
	// - 2 forwarding mocks under google/cloud/kms/mocks/
	// - 28 public v1 headers and sources under google/cloud/kms/v1/ (7 per service)
	// - 4 mock connection headers under google/cloud/kms/v1/mocks/ (1 per service)
	// - 80 internal implementation files under google/cloud/kms/v1/internal/ (20 per service)
	const expectedFiles = 122
