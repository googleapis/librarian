...
func TestComputeAddresses(t *testing.T) {
	...
	computeProtosDest := filepath.Join(tmpGoogleapis, "google", "cloud", "compute")
	computeProtosSrc := filepath.Join(cppRepo, "protos", "google", "cloud", "compute")
	if err := os.RemoveAll(computeProtosDest); err != nil {
		t.Fatal(err)
	}
	if err := os.CopyFS(computeProtosDest, os.DirFS(computeProtosSrc)); err != nil {
		t.Fatal(err)
	}
	...
	library := &config.Library{
		Name:                "compute_addresses",
		Output:              outDir,
		APIs:                []*config.API{{Path: "google/cloud/compute/addresses/v1"}},
		...
		Cpp: &config.CppLibrary{
			...
			GenerateGrpcTransport: &generateGrpc,
		},
	}
	...
	refBase := filepath.Join(cppRepo, "google", "cloud", "compute", "addresses", "v1")
	...
	// The 26 expected files comprise:
	// - 9 public v1 headers and sources under google/cloud/compute/addresses/v1/
	// - 1 mock connection header under google/cloud/compute/addresses/v1/mocks/
	// - 16 internal implementation files under google/cloud/compute/addresses/v1/internal/
