...
type GoInternalCopy struct {
	...
	// Plugins lists the additional protoc plugins to run for the copy.
	Plugins []*GoProtocPlugin `yaml:"plugins,omitempty"`
...
// GoProtocPlugin describes a protoc plugin invocation.
type GoProtocPlugin struct {
	// Name is the plugin name without the "protoc-gen-" prefix, for example
	// "go-vtproto". The binary is looked up in the Go tool bin directory, so
	// declare the Go module providing it under tools.go, and then on the PATH.
	Name string `yaml:"name"`
	// Options are the plugin options, passed as `--<name>_opt` values. The
	// plugin must write its output next to protoc-gen-go's, under the Go
	// import path. Layout options `paths=`, `module=`, and `M<file>=` import
	// mappings are rejected, including in comma-separated parameter lists.
