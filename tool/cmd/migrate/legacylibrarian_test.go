...
func TestSyncVersionOrCopy(t *testing.T) {
	for _, test := range []struct {
		name     string
		existing []*config.Library
		new      []*config.Library
		want     []*config.Library
	}{
		{
			name: "update versions and append new libraries",
			existing: []*config.Library{
		...
		{
			name: "all existing libraries updated",
			existing: []*config.Library{
				{Name: "lib1", Version: "1.0.0"},
				{Name: "lib2", Version: "2.0.0"},
			},
		...
		},
		{
			...
			existing: []*config.Library{{Name: "lib1", Version: "1.0.0"}},
			new:      nil,
			want:     []*config.Library{{Name: "lib1", Version: "1.0.0"}},
		},
	} {
		t.Run(test.name, func(t *testing.T) {
			existingCfg := &config.Config{Libraries: test.existing}
			newCfg := &config.Config{Libraries: test.new}
			got := syncVersionOrCopy(existingCfg, newCfg)
			if diff := cmp.Diff(test.want, got); diff != "" {
				t.Errorf("syncVersionOrCopy() mismatch (-want +got):\n%s", diff)
			}
		})
	}
}
