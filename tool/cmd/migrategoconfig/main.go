...
func main() {
	...
	if cfg.Default.Go == nil {
		cfg.Default.Go = &config.GoDefault{}
	}
	cfg.Default.Go.DefaultEnabledGeneratorFeatures = commonFeatures
	...
	for _, lib := range cfg.Libraries {
		...
		for _, api := range lib.APIs {
			for _, goAPI := range lib.Go.GoAPIs {
				if goAPI.Path == api.Path {
					goAPICopy := *goAPI
					goAPICopy.Path = ""
					if slices.Equal(goAPICopy.EnabledGeneratorFeatures, commonFeatures) {
		...
		lib.Go.GoAPIs = nil
		if slices.Equal(lib.Go.DefaultEnabledGeneratorFeatures, commonFeatures) {
			lib.Go.DefaultEnabledGeneratorFeatures = nil
		}
