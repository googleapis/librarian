...
1.  Follow instructions in your languages’ playbook section for prerequisites (note: this list is not exhaustive).
...
2. **Modify `librarian.yaml` to point to new or local versions.**
    - For example, if you want to bump the protoc-gen-java_grpc, you can change its `version` property.
    - If you want to test a version of the go generation, you can change its `version` property to your branch or commit-hash.
    - For Java, librarian.yaml is configured to build the gapic generator from local path. So you don’t need modifications to test local changes.
